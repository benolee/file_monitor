require "spec_helper"
describe FlexCorp::FileMonitor::Configuration do
  context "#new" do
    context "given a valid path" do
      ...
      it "should parse and have convenience methods for each settings" do
        @config = FlexCorp::FileMonitor::Configuration.new config_path
    ...
    context "given a invalid path" do
      it "should raise FileNotFoundError" do
        lambda{ FlexCorp::FileMonitor::Configuration.new("/invalid/path") }.should raise_error(Errno::ENOENT)
      end
    end
  end
end
